...
class ImageGenerator:
    ...
    def Type_1(self, num, save=False):
        ...
        for i, Iter in enumerate(range(num)):
            ...
            if save:
                cv2.imwrite(self.save_path + label + ".png", Plate)
                # print(self.save_path)
                # print(type(self.save_path))
                if not cv2.imwrite(self.save_path + label + ".png", Plate):
                    raise Exception("Could not write image - type 1")
            else:
    ...
    def Type_2(self, num, save=False):
        ...
        for i, Iter in enumerate(range(num)):
            ...
            if save:
                ...
                if not cv2.imwrite(self.save_path + label + ".jpg", Plate):
                    raise Exception("Could not write image - type 2")
            else:
                cv2.imshow(label, Plate)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
    ...
    def Type_3(self, num, save=False):
        ...
        for i, Iter in enumerate(range(num)):
            ...
            if save:
                cv2.imwrite(self.save_path + label + ".jpg", Plate)
                if not cv2.imwrite(self.save_path + label + ".jpg", Plate):
                    raise Exception("Could not write image - type 3")
            else:
                cv2.imshow(label, Plate)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
